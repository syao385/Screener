    ...
    function switchMainView(view) {
        document.getElementById('nav-tab-screener').classList.remove('active');
        document.getElementById('nav-tab-portfolio').classList.remove('active');
        document.getElementById('nav-tab-macro').classList.remove('active');

        document.getElementById('view-screener-section').style.display = (view === 'screener') ? 'block' : 'none';
        document.getElementById('view-portfolio-section').style.display = (view === 'portfolio') ? 'block' : 'none';
        document.getElementById('view-macro-section').style.display = (view === 'macro' || view === 'screener') ? 'block' : 'none';

        if (view === 'screener') document.getElementById('nav-tab-screener').classList.add('active');
        if (view === 'portfolio') document.getElementById('nav-tab-portfolio').classList.add('active');
        if (view === 'macro') document.getElementById('nav-tab-macro').classList.add('active');
    }
...
/).map(l => l.trim()).filter(l => l.length > 0);
        ...
        for (let i = headerIdx + 1; i < lines.length; i++) {
            ...
            // Check for cash line
            if (symRaw.includes('SPAXX') || desc.toUpperCase().includes('MONEY MARKET') || accNm.includes('Pending activity') || desc.includes('Pending activity')) {
            ...
            if (!symRaw || symRaw.includes('The data and information') || symRaw.includes('Brokerage services') || symRaw.includes('Date downloaded')) {
                continue;
            }
            ...
            const isOption = symRaw.startsWith('-') || desc.toUpperCase().includes('CALL') || desc.toUpperCase().includes('PUT');
            const cleanSym = symRaw.replace(/[\s-]/g, '');
        ...
        const totalEquity = positions.reduce((acc, p) => acc + p.current_value, 0);
        ...
        window.portfolioData = {
            ...
            positions: positions,
            total_day_pnl_dollar: +positions.reduce((acc, p) => acc + p.today_pnl_dollar, 0).toFixed(2),
            total_day_pnl_pct: totalNav > 0 ? +(positions.reduce((acc, p) => acc + p.today_pnl_dollar, 0) / totalNav * 100).toFixed(2) : 0.0,
            total_unrealized_pnl_dollar: +positions.reduce((acc, p) => acc + p.total_pnl_dollar, 0).toFixed(2),
        ...
        alert('✅ Successfully imported ' + positions.length + ' positions from Fidelity!
' +
